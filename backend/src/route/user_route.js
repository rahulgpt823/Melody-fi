...
if(user){
    return res.status(403).json({Error:"A user already with this mail exist , kindly check the maill"});
}
 // create a new user in Db
else{
    // we convert the password to an HASH before storing that in db
    const hashedPassword= await bcrypt.hash(password,10);
    const newUserData= {email,password:hashedPassword,firstname,lastname,DOB,username}
