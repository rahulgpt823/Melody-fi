...
if(validateUserExist){
    ...
    const isValidPassword = bcrypt.compare(validateUserExist.password,password)
    ...
    // if(userHashedPassword === validateUserExist.password){
        ...
        console.log(`User already registerd and token generated to access the app.TOKEN : ${userToken}`);
    ...
    return res.status(403).send(`User not registerd. Kindly register to enjoy the music`)
}
